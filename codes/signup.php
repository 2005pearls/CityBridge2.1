...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (
        ...
        empty($job_title) || empty($email) || empty($phone) || empty($username) ||
        ...
        $error = "Password must contain one uppercase letter, one lowercase letter, one number, and one special character.";
        ...
        try {
            ...
            if ($checkResult->num_rows > 0) {
                $error = "This username or email is already in use. Please log in or choose another one.";
            } else {
                ...
                $_SESSION["flash"] = "Registration successful! Welcome to CityBridge, " . $first_name . ".";
...
<body<?php echo !empty($error) ? ' data-flash-error="'.htmlspecialchars($error, ENT_QUOTES, "UTF-8").'"' : ''; ?>>
